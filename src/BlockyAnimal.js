...
function main() {
  ...
  g_projMat.setPerspective(50, canvas.width / canvas.height, 1, 200);
  ...
  if(slideX){
    slideX.addEventListener('input', function(){
      g_lightSliderX = parseFloat(this.value) / 20.0; 
      renderScene();
    });
  }
  if(slideY){
  ...
  if(slideZ){
    slideZ.addEventListener('input', function(){
      g_lightSliderZ = parseFloat(this.value) / 20.0;
      renderScene();
    });
  }
...
function updateAnimationAngles(){
  if(g_Animation){
    let animX = 2 * Math.cos(g_seconds);
    let animZ = 2 * Math.sin(g_seconds);
    g_lightPos[0] = animX + g_lightSliderX;
    g_lightPos[2] = animZ + g_lightSliderZ;
  } else {
    g_lightPos[0] = g_pauseBaseX + g_lightSliderX;
    g_lightPos[2] = g_pauseBaseZ + g_lightSliderZ;
  }
}
...
function toggleLightAnimation(){
  if(g_Animation){
    g_pauseBaseX = 2 * Math.cos(g_seconds);
    g_pauseBaseZ = 2 * Math.sin(g_seconds);
    g_pausedTime = g_seconds;
    g_Animation = false;
  } else {
    g_startTime = performance.now()/1000.0 - g_pausedTime;
    g_Animation = true;
  }
}
